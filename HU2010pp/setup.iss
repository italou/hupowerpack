#define GameName "Hunting Unlimited 2010"
#define MyAppVersion "1.01"
#define MySetupFile "HU2010pp"
#define InstIcon "pp2010.ico"
#define UninstIcon "pp2010u.ico"
#define HUexe "hu2010.exe"
...
DefaultGroupName=Break For Games\{#GameName}
...
Source: "base\*"; DestDir: "{tmp}\pp10files"; Flags: recursesubdirs ignoreversion deleteafterinstall
...
Source: "..\inno_files\pp2010.ico"; DestDir: "{app}"; Flags: ignoreversion
Source: "..\inno_files\pp2010u.ico"; DestDir: "{app}"; Flags: ignoreversion
...
[Run]
Filename: "{tmp}\7za.exe"; Parameters: "a -tzip -r0 -mx0 ""{code:GetHUPath}\base.scs"" .\pp10files\*"; StatusMsg: "Adding files to your {#GameName} game, please wait..."; Flags: runhidden
...
begin
  ...
  end else if RegQueryStringValue(HKCU, 'Software\Oberon Media\2011034', 'LaunchPath', KeyValue) then begin
    HUPath := KeyValue;
    Result := true;
  // Check & find game folder via Reg - Steam version
  end else if RegQueryStringValue(HKLM, 'Software\Microsoft\Windows\CurrentVersion\Uninstall\Steam App 12690', 'InstallLocation', KeyValue) then begin
    HUPath := KeyValue;
    Result := true;
  end else if RegQueryStringValue(HKLM, 'Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Steam App 12690', 'InstallLocation', KeyValue) then begin
    HUPath := KeyValue;
    Result := true;
  ...
  if (CurUninstallStep = usPostUninstall) then begin
    if DirExists(HUPath + '\pp_files\backup') then begin
      if FileExists(HUPath + '\pp_files\backup\ppbfile.001') or FileExists(HUPath + '\pp_files\backup\ppbfile.002') then begin
        MsgBox('Backup files detected, they will now be restored.'#13#13'Click OK to continue.', mbInformation, mb_OK);
      end;
